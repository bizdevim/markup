...
class vanillaSelectBox {
	constructor(domSelector, options) {
		...
		if (options) {
			...
			if (options.remote != undefined && options.remote) {
				...
				if (options.remote.onInit != undefined &&
				...
				if (options.remote.onSearch != undefined &&
			...
			if (options.maxOptionWidth != undefined &&
		...
		this.createTree = function () {
			...
			if (this.search) {
				...
				if (this.maxOptionWidth < Infinity) {
					this.searchZone.style.maxWidth =
						self.maxOptionWidth + 30 + "px";
			...
			if (document.querySelector(self.domSelector + " optgroup") !== null) {
				...
				self.options = document.querySelectorAll(
					self.domSelector + " option"
				);
				let groups = document.querySelectorAll(
					self.domSelector + " optgroup"
				);
					...
					if (self.isSearchRemote) {
						...
						if (selectAll) {
							...
							if (nrChecked !== nrFound) {
								...
								selectAll.innerText =
									self.userOptions.translations.selectAll;
								selectAll.setAttribute("data-selected", "false");
							} else {
								selectAll.classList.add("active");
								selectAll.innerText =
									self.userOptions.translations.clearAll;
	...
	optionsCheckedToData() {
		let self = this;
		let dataChecked = [];
		let treeOptions = self.ul.querySelectorAll(
			"li.active:not(.grouped-option)"
		);
		let keepParents = {};
		if (treeOptions) {
	...
	reloadTree() {
		...
		if (self.isOptgroups) {
			...
			self.listElements = this.drop.querySelectorAll(
				"li:not(.grouped-option)"
			);
		} else {
				...
				if (value != "all") {
					...
					if (x.hasAttributes()) {
						originalAttrs = Array.prototype.slice
							...
							.filter(function (a) {
								return (
									self.forbidenAttributes.indexOf(a.name) === -1
								);
							});
					}
					let classes = x.getAttribute("class");
